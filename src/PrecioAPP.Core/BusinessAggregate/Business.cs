...
public class Business(string name, string description, string address, int longitude, int latitude, string phone, string email, string website, string logoURL) : EntityBase, IAggregateRoot
{
  ...
  public int Longitude { get; private set; } = Guard.Against.OutOfRange(longitude, nameof(longitude), -180, 180);
  public int Latitude { get; private set; } = Guard.Against.OutOfRange(latitude, nameof(latitude), -90, 90);
  ...
  public void UpdateLongitude(int newLongitude) => Longitude = Guard.Against.OutOfRange(newLongitude, nameof(newLongitude), -180, 180);
  public void UpdateLatitude(int newLatitude) => Latitude = Guard.Against.OutOfRange(newLatitude, nameof(newLatitude), -90, 90);
